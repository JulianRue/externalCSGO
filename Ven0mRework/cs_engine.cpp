...
{
    return mem.read<int>(cs_nv_dwClientState + cs_nv_m_dwGetLocalPlayer);
}

vec3 engine::GetViewAngles(void)
{
    return mem.read<vec3>(cs_nv_dwClientState + cs_nv_m_dwViewAngles);
}

void engine::SetViewAngles(vec3 v)
{
	mem.write<vec3>(cs_nv_dwClientState + cs_nv_m_dwViewAngles, v);
}

int engine::GetMaxClients(void)
{
    return mem.read<int>(cs_nv_dwClientState + cs_nv_m_dwMaxClients);
}

bool engine::IsInGame(void)
{
    return mem.read<unsigned char>(cs_nv_dwClientState + cs_nv_m_dwState) >> 2;
}

cs_player entity::GetClientEntity(int index)
{
    return mem.read<cs_player>(cs_nv_dwEntityList + index * 0x10);
}
...
{
    return cs_vt_entity.address() - (mem.read<uint32_t>(cs_vt_entity.function(5) + 0x22) - 0x38);
}

static csptr_t offset_clientstate(void)
{
    return mem.read<csptr_t>(cs_vt_engine.function(18) + 0x16);
}
...
static void initialize_nv(void)
{
    ...
    cs_nv_m_dwGetLocalPlayer  = mem.read<uint32_t>(cs_vt_engine.function(12) + 0x16);
    cs_nv_m_dwViewAngles      = mem.read<uint32_t>(cs_vt_engine.function(19) + 0xB2);
    cs_nv_m_dwMaxClients      = mem.read<uint32_t>(cs_vt_engine.function(20) + 0x07);
    cs_nv_m_dwState           = mem.read<uint32_t>(cs_vt_engine.function(26) + 0x07);
    cs_nv_m_dwButton          = mem.read<uint32_t>(cs_vt_inputsystem.function(15) + 0x21D);
    cs_nv_m_dwAnalogDelta     = mem.read<uint32_t>(cs_vt_inputsystem.function(18) + 0x29);
    cs_nv_m_dwAnalog          = mem.read<uint32_t>(cs_vt_inputsystem.function(18) + 0x09);
